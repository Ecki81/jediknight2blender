...
def import_3do(file, xOffs, yOffs, zOffs, pitchOffs, yawOffs, rollOffs):
    ...
    while midx < meshCount:
        ...
        ob.location = (x + xOffs, y + yOffs, z + zOffs)     # wrong! translation has to be relative to parent object. this only works in hierarchy with depth of 1

        ob.rotation_euler.rotate_axis("X", radians(pitch + pitchOffs))              # Correct order of local rotation axes (pitch, yaw, roll)
        ob.rotation_euler.rotate_axis("Z", radians(yaw + yawOffs))
        ob.rotation_euler.rotate_axis("Y", radians(roll + rollOffs))
...
class Thing:
    ...
    def importThing(file, x, y, z, pitch, yaw, roll):
        xOffs = x
        yOffs = y
        zOffs = z
        pitchOffs = pitch
        yawOffs = yaw
        rollOffs = roll
        import_3do(file, xOffs, yOffs, zOffs, pitchOffs, yawOffs, rollOffs)
